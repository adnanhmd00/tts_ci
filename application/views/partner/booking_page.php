    ...
    <body class="kt-quick-panel--right kt-demo-panel--right kt-offcanvas-panel--right kt-header--fixed kt-header-mobile--fixed kt-subheader--enabled kt-subheader--fixed kt-subheader--solid kt-aside--enabled kt-aside--fixed" style="">
                        ...
                        <div class="kt-subheader  kt-grid__item" id="kt_subheader">
                            <div class="kt-container  kt-container--fluid ">
                                <div class="kt-subheader__main">
                                    <h3 class="kt-subheader__title">Booking Page</h3>
                                            ...
                                            <div class="kt-portlet__head-label">
                                                <h3 class="kt-portlet__head-title">
                                                        <?php 
                                                            ...
                                                            $currency=$data['deal_data']->currency;
															 if($currency!=$userinfo->currency){
																$point=$CI->admin->getRawRow("Select * from currency_tracker where currency_to='$currency' and currency_from= '$userinfo->currency'")->points;
																$price=$data['deal_data']->price_b2c*$point;
																$symbol=$CI->admin->getRawRow("Select * from tbl_currency where id='$userinfo->currency'")->symbol;
															}else{
																$price=$data['deal_data']->price_b2c;
                                                                $symbol=$CI->admin->getRawRow("Select * from tbl_currency where id='$userinfo->currency'")->symbol;
                                                            }
														?>
                                                    <?= $data['deal_data']->package_name;?> ( <?= $data['deal_data']->days;?>D / <?= $data['deal_data']->nights;?>N) Booking Amount: <?= $symbol;?>.  <?= $price;?> 
                                                </h3>
                                            </div>
                                        </div>
                                        <!--begin::Form-->
                                        <?php $hidden=['deal_id'=>$deal_id];?>
                                        <?php echo form_open_multipart('partner-booking-form', 'class="kt-form"',$hidden);?>
                                            ...
                                            <p><?php echo $this->session->flashdata('item'); ?></p>
                                                ...
                                                <div class="kt-section kt-section--first">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="customer_last_name" required class="form-control"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Email</label>
                                                    ...
                                                    <div class="form-group">
                                                        <label>Deal Dates</label>
                                                        <select id="travel_date" required name="travel_date" class="form-control" >
                                                        <option value="">-Choose-</option>
                                                            <?php foreach($data['deal_dates'] as $row){?>

                                                                <?php 
                                                                    $CID =& get_instance(); $CID->load->model('MainModel','admin'); 
                                                                    $currency_=$row->currency;
                                                                    if($currency!=$userinfo->currency){
                                                                        $point_=$CI->admin->getRawRow("Select * from currency_tracker where currency_to='$currency' and currency_from= '$userinfo->currency'")->points;
                                                                        $price_=$row->price_b2c*$point;
                                                                        $symbol_=$CI->admin->getRawRow("Select * from tbl_currency where id='$userinfo->currency'")->symbol;
                                                                    }else{
                                                                        $price_=$row->price_b2c;
                                                                        $symbol_=$CI->admin->getRawRow("Select * from tbl_currency where id='$userinfo->currency'")->symbol;
                                                                    }
                                                                ?>
                                                                <option value="<?= $row->dates.'_'.$price_;?>"><?=$row->dates;?> ( <?= $symbol_.". ".$price_;?> )</option>
                                                            <?php }?>
                                                        </select>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Return Date</label>
                                                        <input id="return_date" readonly type="text" name="return_date" required class="form-control"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>PAX</label>
                                                        <input  type="number" name="pax" id="pax" required class="form-control"/>
                                                        <input  type="hidden" value="<?= $price;?>" id="deal_price" required class="form-control"/>
                                                    </div>
                                                    <!-- <div class="form-group">
                                                        <label>Amount To Be Paid</label>
                                                        <input readonly type="text" name="price"  id="price"  required class="form-control"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Paying Right Now</label>
                                                        <input  type="text" name="payment_of" required class="form-control"/>
                                                    </div> -->
                                                    <div class="form-group">
                                                        <div class="kt-radio-inline">
                                                            <label class="kt-radio">
                                                                <input type="radio" value="partial" name="payment_of"> Book now with <?= $symbol;?>.  <?= $price;?>
                                                                <span></span>
                                                            </label>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="kt-radio-inline">
                                                            <label class="kt-radio">
                                                                <input type="radio" value="full" name="payment_of"> <label id="complete_payment_label">Complete Payment</label>
                                                                <span></span>
                                                            </label>
                                                        </div>
                                                    </div>
                                                    <!-- <div class="form-group">
                                                        <input type="radio" name="payment_of" /><label id="complete_payment">Paying Right Now</label>
                                                        <input type="radio" name="payment_of" /><label id="partial_payment">Paying Right Now</label>
                                                        <input  type="text" name="payment_of" required class="form-control"/>
                                                    </div> -->
                                                </div>
                                            </div>
        ...
        <script>
            $('#travel_date').change(function() {
                ...
                $.ajax({
					method: "POST",
					url: "<?= base_url();?>Partner/get_next_date/"+gid+"/"+<?= $data['deal_data']->days;?>,
					})
					.done(function( myJSON ) {
								
						$("#return_date").val(myJSON);
				});
            });
        ...
        <script>
            $( document ).ready(function() {
                $("#pax").keyup(function() {
                    var date_and_price=$( "#travel_date option:selected" ).val();

                    price_=date_and_price.split("_")
                    var p=price_[1];
                    var pax=$("#pax").val();
                    var price = pax*p;
                    $("#price").val(price);
                    $("#complete_payment_label").html("Make Complete Payment Of <?= $symbol;?>. "+price) 
                });
            });
        </script>
